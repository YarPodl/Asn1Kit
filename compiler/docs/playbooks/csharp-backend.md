...
7. **Алиасы.** Typedef без constructed RHS, без `namedBits` и без `enumerated` **не эмитится**: `UnwrapAliases` подставляет исходный тип в `CsType` / encode / decode; на поле — `/// <summary>ASN.1 alias Name ::= …</summary>`.
