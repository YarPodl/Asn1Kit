...
**Последствие.** В шаблонах бэкенда нет байтов тегов и длин — только вызовы `Asn1Writer` / `Asn1Reader` / `Asn1Primitives`. Сгенерированный код тестируется компиляцией через Roslyn в `RoundTripTests`.
...
**Последствие.** Runtime пишет DER-форму по X.690 §11.7 (`YYMMDDHHMMSSZ` / `YYYYMMDDHHMMSS[.f…fffffff]Z`: секунды обязательны, суффикс `Z`, точка, без хвостовых нулей дроби; нулевая дробь опускается). На чтении дробь принимается только длины 1…7; BER дополнительно допускает пропуск секунд, смещение `±hhmm` и запятую. Год UTCTime: `00–49` → 20xx, `50–99` → 19xx. Исходная строка не сохраняется — точный re-encode подписанного значения не гарантируется, если вход был не-DER.
