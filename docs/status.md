...
| `choice`                        | да                                                                   | да → класс + enum `…Kind` + `From…` фабрики; **один** вариант → сворачивается как алиас (без класса/`Kind`)                           | `ParserTests`, `PkixExplicit88Tests`, `PkixGeneratedCodeTests`, `RoundTripTests.GeneratedCSharp_CollapsesSingleAlternativeChoice`                 |
