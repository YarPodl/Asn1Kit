...
На **той же машине** (тот же OS/.NET/CPU) после оптимизаций Asn1Kit достаточно гонять только методы `Asn1Kit_*`: BCL и BouncyCastle от железа не меняются, их цифры бери из полного baseline ([2026-09-23](2026-09-23/) или новее с `--filter *`).
...
В `SUMMARY.md`: git SHA, фильтр (`*Asn1Kit*` или `*`), и таблицы Asn1Kit; для peers укажи ссылку на полный baseline, если прогон был только Asn1Kit.
